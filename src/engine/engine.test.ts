...
describe('deterministic card engine', () => {
  it('creates a complete unique deck and repeats a seeded shuffle', () => {
    const a = shuffledDeck('example', 2),
      b = shuffledDeck('example', 2);
  ...
  it('offers empty FreeCell destinations and accepts foundation moves from cells', () => {
    ...
    const toCell = freecell
      .legalMoves(initial)
      .find((move) => move.to === 'c0' && move.from.startsWith('t'));
    ...
    const source = Object.values(forced.piles).find(
      (p) => p.kind === 'tableau' && p.cards.some((card) => card.rank === 1),
    )!;
    ...
    expect(freecell.legalMoves(forced)).toContainEqual({
      type: 'transfer',
      from: 'c0',
      to: 'f0',
      cardIds: [ace.id],
