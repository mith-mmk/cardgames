...
| --------------------------- | ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Engine contract             | Pass                           | `src/engine/gameContract.test.ts` enumerates all 100 registered games, validates deterministic deals, full unique decks, valid/invalid moves, legal hints, Undo, and same-seed Retry.                                                                     |
| 100-game UI smoke           | Pass                           | `e2e/game-smoke.spec.ts` starts every catalog game on desktop and 844×390 coarse-pointer layouts, checks the initial deal, Help/Hint controls, one engine-selected legal UI move, Undo, Retry, and horizontal overflow.                                   |
| Existing browser regression | Pass                           | The 0.6.10 rerun covers the Bowling selected-ball touch target at 568×320 and 844×390, plus the localized help journey; both browser regressions are resolved.                                                                                            |
| UI evidence                 | Pass with device follow-up     | Coarse-pointer browser checks verify the remediated Bowling board at representative landscape sizes. Physical iPhone/iPad safe-area confirmation remains a platform validation item.                                                                      |
| Android native target       | Pass with device follow-up     | The versioned Tauri shell now validates its identifier/version, landscape-only activity, cutout/keyboard handling, dark edge-to-edge system bars, and release clear-text policy. An Android SDK device/emulator run remains the platform validation item. |
...
| Resolved | Compact score grids covered the action row           | The compact action row now has its own stacking layer above scrollable score grids, so Hint and session actions remain tappable at 844×390 without changing tablet grid sizing.                                                                  |
| Resolved | Bowling's seven actions formed an off-screen row     | Compact landscape gives Bowling Solitaire a seven-column action row, keeping Knock, Next ball, Hint, Auto-complete, Undo, Retry, and New game inside the viewport.                                                                               |
...
All 100 `GAME_DEFINITIONS` are covered by deterministic engine and rendered-UI smoke tests. Rule-complete status is grouped below so a green smoke test is not mistaken for game-complete status.

The remaining per-game playable-rule acceptance fixtures are tracked here rather than as a UI/accessibility issue. The smoke gate proves one legal interaction, Hint, Undo, and Retry for every game; it does not substitute for a full human playthrough or a published-rule comparison.
...
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Resolved 0.6.10 | Bowling Solitaire's selected active ball was not the top hit target in touch emulation.                                                          | `bowlingActive` now has a dedicated high stacking layer. Compact wide boards also restore the table to the first grid row, so the 844×390 layout cannot collapse it behind the controls. E2E asserts the visible hit target at 568×320 and 844×390. | No further web remediation required.                                                                                                                                   |
| Resolved 0.6.10 | The localized help E2E used the English catalog title after switching to Japanese, and compact layout removed the back button's accessible text. | The test now selects `クロンダイク` in Japanese. The back button has an explicit localized `aria-label`, preserving its accessible name while its visual label is hidden.                                                                           | No further remediation required.                                                                                                                                       |
...
- `npm run lint` and all 596 Vitest tests pass.
- The full Playwright suite passes: 88 tests pass and 2 desktop-only compact-layout checks are intentionally skipped. The mobile project confirms the active Bowling ball is the actual hit target at 568×320 and 844×390.
- PWA artifact validation passes, and Android/iOS static checks pass. iOS runtime packaging remains unavailable on this Windows host.
- `npm run tauri:android:verify` validates the Android native shell. Android device/emulator packaging is intentionally not claimed without the required SDK/NDK and signing environment.
- `build-windows.bat` produced the 0.6.10 x64 NSIS and MSI installers, then regenerated the final Web/PWA service-worker artifacts.

### 100-game smoke verification

- The engine contract now contains 596 checks, including invalid-move rejection, legal Hint, Undo, and deterministic Retry across all 100 games.
- `e2e/game-smoke.spec.ts` exercises every registered catalog tile and the engine-selected opening UI move on full-HD desktop and 844×390 coarse-pointer landscape. It verifies the exact card-to-pile arrangement after the move, Undo, and Retry.
...
## iOS follow-up — Clock, layout, and icon — 2026-08-21

- **Resolved P1 — Clock rule/display mismatch:** An exposed Clock card is now placed into the matching hour's completed-card pile rather than a single, overlapping removed pile. Tapping the exposed card advances the forced move, hour labels are visible, and a depleted next source produces a loss. Engine tests retain all 52 cards and cover the exhausted-source case.
- **Resolved P1 — iPad Clock card layout:** Completed cards sit behind their associated source pile, clock positions use a compact iPad-safe layout, and the `mobile-chrome` E2E test at 1024×768 verifies all 13 source piles remain inside the table and that a tap reaches the selected hour.
- **Resolved P1 — iOS local-network and icon assets:** The plist now declares local-network development use. The canonical icons in `src-tauri/icons/ios` are synchronized into the generated `AppIcon.appiconset` before every iOS dev/simulator build; `tauri:ios:verify` compares their hashes to prevent the generated default icon from returning.
- **Resolved P1 — iPad landscape-only enforcement:** `src-tauri/src/lib.rs` now applies landscape-only support directly to Tauri's native iOS view controller after the WebView exists. This prevents Tauri's default landscape-and-portrait controller setting from overriding the plist policy during startup.
- **Open P1 — iPadOS 26 cannot be made to visually rotate by a release-safe app API:** iPadOS 26 can adjust a landscape-only app scene without visually rotating the physical display or a raw screenshot frame. Apple documents this behavior for `UIRequiresFullScreen`; private orientation-forcing APIs would make the app unsuitable for release. The current release-safe behavior is a landscape-only scene; either accept this iPadOS presentation or change the product requirement to support dynamically resized, all-orientation iPad scenes.
- **Validation:** unit suite (497 tests), lint, production build, iOS plist/icon verification, Clock mobile E2E, and the arm64 iOS Simulator archive pass. The full E2E suite currently retains the unrelated Bowling touch-hit-target failure and two persisted-locale test-isolation failures documented above.
